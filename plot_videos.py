...
def writeAudio(vid_loc, audio_loc):
    ...
    cmd = f'ffmpeg -loglevel panic -i "{vid_loc}" -i "tmp.wav" -c:v copy -c:a aac -strict experimental "{new_vid_loc}"' #temporary .wav audio location
    ...
    try:
        os.remove(vid_loc)  # delete the original video file without audio
    except PermissionError:
        print("Could not delete video (in use):", vid_loc)
...
def plot_video(joints,
    ...
    npy_folder = os.path.join(os.path.dirname(file_path), "test_poses")  # to store numpy files for output poses (optional)
    ...
    if audio_path is not None and os.path.exists(audio_path):
        ...
        print(f"Audio not found at {audio_path}, creating video without sound.")
...
def draw_frame_2D(frame, joints):
    ...
    # FIX 1: Ensure the offset array matches your 50 joints
    joints = joints + np.ones((50, 2)) * offset

    # Loop through each of the bone structures, and plot the bone
    for j in range(number):
        # FIX 2: Get the indices for the start and end of the bone
        joint_start = skeleton[j, 0]
        joint_end = skeleton[j, 1]

        # FIX 3: Critical check! Only draw if both indices exist in our 50-joint data
        if joint_start < 50 and joint_end < 50:
            c = get_bone_colour(skeleton, j)

            draw_line(frame, [joints[joint_start][0], joints[joint_start][1]],
                      [joints[joint_end][0], joints[joint_end][1]], c=c, t=1, width=3)
