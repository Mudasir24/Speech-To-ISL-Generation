...
class SignProdDataset(data.Dataset):
    ...
    def __init__(self, path, exts, fields, trg_size, src_size, num_sec,
                skip_frames=1, is_train=True, **kwargs):

        if not isinstance(fields[0], (tuple, list)):
            fields = [('src', fields[0]), ('trg', fields[1]),
                    ('nonreg_trg', fields[2]), ('file_paths', fields[3])]
        ...
        def load_variable_length_data(file_path):
            data = []
        ...
        audio_data = load_variable_length_data(path + ".audio")
        skel_data = load_variable_length_data(path + ".skels")
        ...
        for i in range(len(files)):
            ...
            src = audio_data[i]
            trg = skel_data[i]
            nonreg_trg_line = texts[i]

            # ================= AUDIO =================
            valid_len = (len(src) // src_size) * src_size
            if valid_len == 0:
                continue

            src = src[:valid_len]
            src = src.reshape(-1, src_size)

            # ================= SKELETON =================
            valid_len = (len(trg) // trg_size) * trg_size
            if valid_len == 0:
                continue

            trg = trg[:valid_len]
            trg = trg.reshape(-1, trg_size)

            # 🔥 STRICT CLEANING (Dynamic)
            if trg.shape[1] != trg_size:
                continue

            if src.shape[1] != src_size:
                continue

            # ================= LENGTH FILTER =================
            if len(src) > src_fps * num_sec:
                continue
            ...
            # ================= TRIM TARGET =================
            trg = trg[:num_sec * trg_fps]

            # ================= FINAL SAFETY =================
            # Ensure sequence is long enough to survive the future_prediction slice (10 frames)
            if trg.shape[0] <= 10 or src.shape[0] == 0:
                continue

            # ================= CREATE EXAMPLE =================
            examples.append(data.Example.fromlist(
                [src, trg, nonreg_trg_line, file_name],
                fields
            ))
